...
def inside_ROI(points, roi):
    ...
    return pointArray[res_idx], res_idx
...
def entrance(areaID, area_path, buildingID, slice_num, dbscan_min_dist):
    ...
    sorted_points = points[sorted_idx][::-1] # reverse
    ...
    while(idx < l):
        ...
        block_denote_ind = sorted_idx[last_time_idx:idx]
        ...
        block_instances_denote_ind = [None] * n_clusters_
        # block_instances_roi = [None] * n_clusters_

        for i in range(len(labels)):
            if block_instances[labels[i]] == None:
                block_instances[labels[i]] = []
                block_instances_denote_ind[labels[i]] = []
            block_instances[labels[i]].append(block[i])
            block_instances_denote_ind[labels[i]].append(block_denote_ind[i])
        ...
        if len(building_instances) == 0:
            ...
            building_instances_denote_ind = block_instances_denote_ind.copy()
        else:
            if REACH_BASE:
                for block_inst in block_instances:
                    for roi_idx, roi in enumerate(building_instances_roi):
                        inside_points, inside_points_ind = inside_ROI(block_inst, roi)
                        if len(inside_points) > 0:
                            building_instances[roi_idx] = np.vstack((building_instances[roi_idx], inside_points))
                            building_instances_denote_ind[roi_idx] = np.hstack((building_instances_denote_ind[roi_idx], inside_points_ind))
            else:
                ...
                for block_idx, block_inst_denote_ind in enumerate(block_instances_denote_ind):
                    print(block_idx)
                    building_idx = block2building_idx_map[block_idx]
                    print("block, building:", block_idx, building_idx)
                    if building_idx == SET_NEW:
                        building_instances_denote_ind.append(block_inst_denote_ind)
                    else:
                        building_instances_denote_ind[building_idx] = np.hstack((building_instances_denote_ind[building_idx], block_inst_denote_ind))
    ...
    for id, inst_denote_ind in enumerate(building_instances_denote_ind):
        np.savetxt(area_path + str(buildingID) + "_pred/" + str(buildingID) + "_" + str(id) + "_ind.txt", inst_denote_ind)
...
with open('./top_to_bottom_args.json', 'r') as file:
    ...
    for d in data:
        ...
        for a in d["args"]:
            ...
            dbscan_min_dist = a["dbscan_min_dist"]
            entrance(areaID, area_path, buildingID, slice_num, dbscan_min_dist)